...
Based on 20_DirectionalLight.js
...
- Applying Toon Shading (Cell Shading) using color quantization
...
let toonLevels = 3;  // Default toon levels - corrected variable name

const cylinder = new Cylinder(gl, 1024, { color: [1.0, 0.0, 0.0, 1.0] });
const axes = new Axes(gl, 1.5); // create an Axes object with the length of axis 1.5

// Per assignment requirements
const cameraPos = vec3.fromValues(0, 0, 3);
// Direction vector from point (1.0, 0.25, 0.5) to origin (0,0,0)
const lightDirection = vec3.fromValues(1.0, 0.25, 0.5);
vec3.normalize(lightDirection, lightDirection); // Normalize the direction
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        ...
        // Add keyboard event for toon levels (1-5)
...
function initWebGL() {
    ...
    // Set canvas size to 700x700 as per requirement
    canvas.width = 700;
    canvas.height = 700;
...
function render() {
    ...
    // Set toon levels uniform in the shader
...
async function main() {
    try {
        ...
        // Set initial toon levels
        shader.setInt("toonLevels", toonLevels);

        // Set up text overlays as specified in requirements
